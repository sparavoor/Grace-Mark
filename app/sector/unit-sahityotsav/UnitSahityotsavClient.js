...
export default function UnitSahityotsavClient({ criteria, initialSubmissions, units = [] }) {
  const initialMap = {};
    ...
    <div className="space-y-10 pb-10">
      <FadeInUp className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-8 border-b border-slate-200">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-navy-900 uppercase">
            Unit <span className="text-indigo-600 font-bold">Sahityotsav</span>
          </h1>
          <p className="text-slate-500 font-normal text-sm mt-1">Select completed units to earn Sector Grace Marks automatically.</p>
        </div>
        <div className="flex items-center gap-3 px-6 py-3 bg-navy-900 border border-slate-800 rounded-[10px] text-white font-semibold text-xs shadow-xl shadow-navy-900/20">
          ...
          <span className="uppercase tracking-[0.2em] font-bold">Estimated Grace Score: {totalLiveMarks.toFixed(1)} / 15.0</span>
        </div>
      </FadeInUp>
      ...
      {criteria.length === 0 ? (
        <ScaleIn className="card-premium text-center py-20 border-2 border-dashed border-slate-200">
          ...
          <h3 className="text-xl font-bold text-navy-900 uppercase mb-2">No Active Unit Sahityotsav Criteria</h3>
                ...
                <div className="space-y-6">
                  ...
                  <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl space-y-3">
                    ...
                    {item.shineType === 'NUMBER' ? (
                      <p className="text-[10px] font-semibold text-indigo-700 uppercase leading-none">
                        Goal-based unit scoring! For each unit, input a value that meets or exceeds <span className="font-bold text-indigo-850">{item.targetSteps} steps</span> to count that unit as completed. Overall grade score scales by total completed units.
                      </p>
                    ) : (
                          ...
                          return (
                            <div 
                              ...
                              className={`p-4 border rounded-2xl transition-all duration-300 flex flex-col gap-3 justify-between ${
                                state.isTicked 
                                  ? (targetReached || item.shineType !== 'NUMBER' ? 'bg-indigo-50/40 border-indigo-100/80 shadow-sm shadow-indigo-50' : 'bg-rose-50/30 border-rose-100')
                              ...
                              {state.isTicked && item.shineType === 'NUMBER' && (
                                <div className="flex items-center gap-2 pl-8 animate-fade-in w-full flex-wrap">
                                  <input 
                                    ...
                                    className="w-20 px-2 py-1 text-center font-bold border border-slate-200 rounded-lg text-xs focus:outline-none focus:border-indigo-600 bg-white"
                                    placeholder="Value"
                                  />
                                  <button
                                    onClick={() => handleSaveNumberUnit(item.id, unit.id)}
                                    disabled={state.saving}
                                    className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-[9px] uppercase tracking-wider font-bold transition-all flex items-center gap-1 shrink-0 shadow-sm"
                                  >
