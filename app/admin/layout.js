...
import { LayoutDashboard, Calendar, BarChart3, Users, Network, LogOut, Trophy } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col md:flex-row overflow-hidden">
      ...
      <aside className="hidden md:flex w-72 bg-navy-900 text-white flex-col sticky top-0 h-screen shadow-2xl z-50">
        <div className="p-8 pb-12">
          <div className="flex items-center gap-3 px-2">
            ...
            <span className="text-xl font-bold tracking-tighter">Grace Mark<span className="text-indigo-400">.</span></span>
          </div>
        </div>
        ...
        <nav className="flex-grow px-4 space-y-1">
          {navItems.map((item) => (
            ...
            >
              ...
              <span className="tracking-tight text-sm uppercase font-semibold tracking-widest">{item.name}</span>
            </Link>
          ))}
        </nav>
      ...
      <div className="flex-grow flex flex-col min-w-0 h-screen">
        ...
        <header className="md:hidden bg-white border-b border-slate-200 p-4 flex justify-between items-center sticky top-0 z-40 shadow-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold tracking-tighter text-xs text-navy-900 uppercase">Grace Mark.</span>
          </div>
          <form action="/login" method="POST">
      ...
      <div className="md:hidden handle-safe-area fixed bottom-8 left-1/2 -translate-x-1/2 w-[90%] z-50">
        <nav className="bg-navy-900 backdrop-blur-3xl border border-white/10 rounded-[2rem] shadow-2xl flex justify-around items-center p-3">
          {navItems.map((item) => (
            <Link 
              key={item.href} 
              href={item.href} 
