...
export function calculateGraceMarksCategory(submissions, units = [], activeCounts = { shine: 1, unitSahityotsav: 1, brightUnitSahityotsav: 1 }) {
  ...
  // Parse activeCounts for backward compatibility
  let activeShineCount = 1;
  let activeUnitCount = 1;
  let activeBrightCount = 1;
  
  if (typeof activeCounts === 'object' && activeCounts !== null) {
    activeShineCount = activeCounts.shine ?? 1;
    activeUnitCount = activeCounts.unitSahityotsav ?? 1;
    activeBrightCount = activeCounts.brightUnitSahityotsav ?? 1;
  } else if (typeof activeCounts === 'number') {
    activeShineCount = activeCounts;
  }

  const count = activeShineCount > 0 ? activeShineCount : 1;
  const marksPerCriteria = 20 / count;

  if (submissions && Array.isArray(submissions)) {
    // Helper to check if a unit submission is completed
    const isSubCompleted = sub => {
      if (!sub.isTicked) return false;
      if (!sub.criteria) return false;
      const st = sub.criteria.shineType || 'TICK';
      if (st === 'NUMBER') {
        const target = sub.criteria.targetSteps;
        if (target && target > 0) {
          return (sub.percentage || 0) >= target;
        }
      }
      return true;
    };
    ...
    const unitSahityotsavSubs = submissions.filter(sub => sub.criteria && sub.criteria.isActive && sub.criteria.type === 'UNIT_SAHITYOTSAV' && isSubCompleted(sub));
    const brightUnitSahityotsavSubs = submissions.filter(sub => sub.criteria && sub.criteria.isActive && sub.criteria.type === 'BRIGHT_UNIT_SAHITYOTSAV' && isSubCompleted(sub));
    ...
    if (unitSahityotsavSubs.length > 0 && activeUnitCount > 0) {
      unitSahityotsavTicked = true;
      const totalPossible = activeUnitCount * numUnits;
      // Percentage is (ticked units / total possible) * 100
      unitSahityotsavPercent = Math.round((unitSahityotsavSubs.length / totalPossible) * 100);
    ...
    if (brightUnitSahityotsavSubs.length > 0 && activeBrightCount > 0) {
      brightUnitSahityotsavTicked = true;
      const totalPossible = activeBrightCount * numUnits;
      // Percentage is (ticked units / total possible) * 100
      brightUnitSahityotsavPercent = Math.round((brightUnitSahityotsavSubs.length / totalPossible) * 100);
...
export function calculateGraceMarks(sector, activeCounts = { shine: 1, unitSahityotsav: 1, brightUnitSahityotsav: 1 }) {
  ...
  const grace = calculateGraceMarksCategory(sector.graceMarkSubmissions || [], sector.units || [], activeCounts);
